...
void ping_auto(uint8 s, uint8 *addr)
{
	...
	{
		switch(getSn_SR(0))																			/*获取socket状*/
		{
			case SOCK_CLOSED:																			/*socket关闭状态*/
				close(0);
				IINCHIP_WRITE(Sn_PROTO(0), IPPROTO_ICMP);           /*设置ICMP 协议*/
				if(socket(0,Sn_MR_IPRAW,3000,0)!=0)									/*判断ip raw模式socket是否开启*/
				{ 
			...
			case SOCK_IPRAW:																			/*ip raw模式*/
				ping_request(0, addr);															/*发送Ping请求*/
				req++;
				while(1)
				{	
					if ( (len = getSn_RX_RSR(0) ) > 0)
					{
						ping_reply(0, addr, len);												/*获取回复信息*/
						rep++;
						break;
					}
					else if(cnt > 100)
